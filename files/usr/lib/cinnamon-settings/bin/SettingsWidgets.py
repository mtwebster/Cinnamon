...
class SocketChooserButton (Gtk.Button):
    def __init__ (self, num_cols=4, button_picture_size=None, menu_pictures_size=None, has_button_label=False):
        super(SocketChooserButton, self).__init__()
        self.num_cols = num_cols
        self.button_picture_size = button_picture_size
        self.menu_pictures_size = menu_pictures_size
        self.set_size_request(button_picture_size * 3, button_picture_size)
        self.row = 0
        self.col = 0
        self.inc = 0
        self.menu_item_tracker = 0
        self.menu = Gtk.Menu()
        self.button_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        self.display_socket_holder = Gtk.Frame()
        self.display_socket_holder.set_no_show_all(True)
        self.display_socket_holder.set_visible(False)
        self.spinner = Gtk.Spinner()
        self.button_box.pack_start(self.spinner, True, True, 2)
        self.spinner.start()
        ...
        if has_button_label:
            self.button_label = Gtk.Label()
            self.button_box.pack_start(self.button_label, True, True, 2)
        self.add(self.button_box)
        self.connect("button-release-event", self._on_button_clicked)
        self.progress = 0.0
    ...
    def set_display_widget (self, title):
        old_sock = self.display_socket_holder.get_child()
        if old_sock:
            old_sock.destroy()

        socket = Gtk.Socket()
        self.display_socket_holder.add(socket)
        socket.set_visible(True)
        socket.connect("plug-added", self.on_display_plug_added)
        GObject.timeout_add(5, self.spawn_previewer, title, socket)

    def on_display_plug_added (self, socket):
        self.display_socket_holder.set_no_show_all(False)
    ...
    def _on_widget_selected(self, widget, callback, title):
        result = callback(title)

        if result:
            self.set_display_widget(title)
    ...
    def add_socket(self, callback, title):
        ...
        socket.connect("plug-added", self.on_menu_plug_added)
    ...
    def on_menu_plug_added(self, widget):
        self.menu_item_tracker -= 1
        self.increment_loading_progress(self.inc)
        if self.menu_item_tracker == 0:
            GObject.timeout_add(5, self.reset_loading_progress)
