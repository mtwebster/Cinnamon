...
class ScreenshotApplication(Gtk.Application):
    ...
    def capture(self, mode, include_pointer, delay, on_done, area_rect=None):
        ...
        def do_capture():
            try:
                if monitor_crop:
                    full = self.backend.screenshot(include_pointer)
                    ...
                    x, y, w, h = area_rect
                    pixbuf = self.backend.screenshot_area(x, y, w, h)
                elif mode == 'window':
                    pixbuf = self.backend.screenshot_window(include_pointer, prefs.get_include_shadow())
                else:
                    pixbuf = self.backend.screenshot(include_pointer)
            except Exception as exc:
                print(f'cinnamon-screenshot: capture failed: {exc}', file=sys.stderr)
                pixbuf = None
            on_done(pixbuf)
