...
function xletRequire(path) {
    global.logWarning('DEPRECATION WARNING: require() is deprecated for xlets. Use Extension.getCurrentExtension() instead.');

    // First check if we're in the middle of loading an extension
    ...
    if (path.startsWith('./') || path.startsWith('../')) {
        ...
        // Generate cache key from extension path + module path
        ...
        // Set up module.exports compatibility for this require
        ...
        if (moduleObj.exports !== moduleObj._originalExports) {
            moduleExportsCache[cacheKey] = moduleObj.exports;
    ...
    // Handle gi imports: 'gi.St' -> imports.gi.St
...
function clearXletImportCache(extension) {
    ...
    if (extension.imports && typeof extension.imports.clearCache === 'function') {
        ...
        try {
            // Get all property names and clear each cached module
            ...
            global.logWarning(`Failed to clear import cache for ${extension.uuid}: ${e.message}`);
        }
    }
}
